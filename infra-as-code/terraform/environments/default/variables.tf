...
variable "SERVERLESS_DOMAIN" {
  ...
  validation {
    condition     = length(var.SERVERLESS_DOMAIN) > 35 && substr(var.SERVERLESS_DOMAIN, 0, 35) == "serverless-flex-play-announcements-"
    error_message = "SERVERLESS_DOMAIN expected to start with \"serverless-flex-play-announcements-\"."
  }
}
